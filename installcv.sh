...
(cd "$SOURCE_DIR" && curl -L "https://www.python.org/ftp/python/2.7.10/Python-2.7.10.tar.xz")
...
(cd "$SOURCE_DIR/Python-2.7.10" && make && make install && chmod -v 755 $PREFIX_DIR/lib/libpython2.7.so.1.0)
